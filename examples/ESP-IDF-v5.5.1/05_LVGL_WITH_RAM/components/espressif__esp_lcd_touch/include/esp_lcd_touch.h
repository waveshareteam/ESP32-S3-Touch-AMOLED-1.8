...
/**
 * @brief Touch point data returned by esp_lcd_touch_get_data()
 */
typedef struct {
    uint16_t x;        /*!< X coordinate */
    uint16_t y;        /*!< Y coordinate */
...
/**
 * @brief Read touch point data from touch controller
 *
 * This is a compatibility wrapper for newer ESP LCD touch users. It internally uses
 * esp_lcd_touch_get_coordinates().
 *
 * @param tp Touch handler
 * @param data Array of touch point data
 * @param data_count Count of returned touch points
 * @param max_data_count Maximum number of touch points to return
 *
 * @return
 *      - ESP_OK on success
