...
esp_err_t esp_lcd_touch_get_data(esp_lcd_touch_handle_t tp, esp_lcd_touch_point_data_t *data, uint8_t *data_count, uint8_t max_data_count)
{
    ESP_RETURN_ON_FALSE(tp != NULL, ESP_ERR_INVALID_ARG, TAG, "Touch handle is NULL");
    ESP_RETURN_ON_FALSE(data != NULL, ESP_ERR_INVALID_ARG, TAG, "Touch data is NULL");
    ESP_RETURN_ON_FALSE(data_count != NULL, ESP_ERR_INVALID_ARG, TAG, "Touch data count is NULL");
    ESP_RETURN_ON_FALSE(max_data_count > 0, ESP_ERR_INVALID_ARG, TAG, "Max touch data count is zero");
    ...
    uint8_t point_num = 0;
    uint8_t max_point_num = max_data_count;
    if (max_point_num > CONFIG_ESP_LCD_TOUCH_MAX_POINTS) {
        max_point_num = CONFIG_ESP_LCD_TOUCH_MAX_POINTS;
    }

    bool touched = esp_lcd_touch_get_coordinates(tp, x, y, strength, &point_num, max_point_num);
